...
AS
    ...
    BEGIN
		...
		AND[pvt].[dbid]
		IN(SELECT DB_ID( [MD].[database_name] )
		...
		FROM[oqs].[queries]
		AS[oqs_q]
		INNER JOIN[oqs].[query_stats]
		AS[qs] ON([oqs_q].[query_hash] = [qs].[query_hash]
